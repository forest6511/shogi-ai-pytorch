...
class TestCheckDetection:
    ...
    def test_king_in_double_check_by_rook_and_bishop(self) -> None:
        ...
        assert _is_in_check(board, Player.SENTE), (
            "Sente king at (4,4) should be in double check"
        )
...
class TestLegalMoveFiltering:
    ...
    def test_checkmate_position_has_no_legal_moves(self) -> None:
        ...
        moves = legal_moves(board, Player.SENTE)
        assert len(moves) == 0, (
            f"Cornered king should have 0 legal moves, got {len(moves)}"
        )
...
class TestPromotionRules:
        ...
        for d in moves:
            ...
            if to_row <= 1:
                assert d["promote"] is True, (
                    f"Knight landing on row {to_row} must promote"
                )
    ...
    def test_captured_promoted_piece_reverts_to_base(self) -> None:
        ...
        assert PieceType.ROOK in gote_hand, (
            "Captured Dragon should revert to ROOK in Gote's hand"
        )
    ...
    def test_cannot_drop_pawn_in_column_with_existing_own_pawn(self) -> None:
        ...
        for move in moves:
            ...
            if d["type"] == "drop" and d["piece_type"] == PieceType.PAWN:
                _, drop_col = d["to"]
                assert drop_col != 3, (
                    f"Nifu violation: cannot drop pawn in col 3 where own pawn exists"
                )
    ...
    def test_nifu_for_gote(self) -> None:
        ...
        for move in moves:
            ...
            if d["type"] == "drop" and d["piece_type"] == PieceType.PAWN:
                _, drop_col = d["to"]
                assert drop_col != 6, (
                    "Gote nifu: cannot drop pawn in col 6 where Gote pawn exists"
                )
...
class TestUchifuzumeRule:
    ...
    Uchifuzume (打ち歩詰め) is a special rule in standard shogi that forbids
    delivering checkmate by dropping a pawn. It is distinct from the general
    check-after-move filter because it requires evaluating the opponent's reply.

    The current implementation in moves.py does NOT enforce Uchifuzume.
    The legal_moves() function only filters moves that leave the *current*
    player's own king in check; it does not examine whether a pawn drop gives
    an illegal checkmate to the opponent.

    The xfail test below documents this gap so that when the rule is
    implemented the test automatically flips to a pass (xpass).
    """

    @pytest.mark.xfail(
        reason=(
            "Uchifuzume (打ち歩詰め) is not yet enforced in moves.legal_moves(). "
            "The current implementation only filters moves that leave the current "
            "player's own king in check; it does not detect the special case where "
            "dropping a pawn would deliver an illegal checkmate to the opponent. "
            "This is a known gap — implement Uchifuzume filtering and remove xfail."
        ),
        strict=True,
    )
    def test_uchifuzume_is_illegal(self) -> None:
        ...
        The current implementation allows this drop (the xfail captures that).
        When Uchifuzume is enforced, (1, 8) must not appear in pawn drop destinations.
        """
        board = _make_board(
            [
                (8, 4, PieceType.KING, Player.SENTE),
    ...
    def test_three_random_games_all_terminate(self) -> None:
        ...
        for game_num in range(3):
            ...
            if state.is_terminal:
                winner = state.winner
                assert winner in (0, 1, None), (
                    f"Game {game_num}: unexpected winner value {winner}"
                )
